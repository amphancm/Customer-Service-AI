<template>
      ...
      <div class="h-screen flex flex-col p-4">
        ...
        <div class="flex-grow  py-4 overflow-y-auto mb-2">
          <div v-for="(chatRoom, index) in Object.values(chatRooms).reverse()" :key="index"
            ...
            :class="Object.values(chatRooms).length - 1 - index == selectIndexing
    ...
    <div class="bg-customeChat flex-none w-[260px] ">
      <div class="h-screen flex flex-col p-4 ">
        ...
        <div class="flex-grow  py-4 overflow-y-auto mb-2">
          <div v-for="(chatRoom, index) in Object.values(chatRooms).reverse()" :key="index"
            @click="selectRoom(Object.values(chatRooms).length - 1 - index)"
            class="w-full h-14 border  rounded-lg mb-2 flex items-center px-4 overflow-x-scroll cursor-pointer" :class="Object.values(chatRooms).length - 1 - index == selectIndexing
              ? 'bg-text_bg_color_message_response text-red_jetts font-bold'
              : 'bg-white'
              ">
            <p>{{ chatRoom.sender.displayName }}</p>
          </div>
        ...
        <!-- <div class="flex-grow py-4 overflow-y-auto mb-2">
          <div v-for="(chatRoom, index) in Object.values(chatRooms).reverse()" :key="index"
            @click="selectRoom(Object.values(chatRooms).length - 1 - index)"
            class="w-full h-14 rounded-lg mb-2 flex items-center px-4 overflow-x-scroll cursor-pointer" :class="[
        ...
        <!-- <div class="flex-none">
          <button class="bg-red_jetts text-white px-4 py-2 rounded-full  w-full" @click="openCreateModalSystemPrompt()">
            Create Room
          </button>
        </div> -->
      </div>
    ...
    <div class="flex-auto w-full">
      <div class="h-screen flex flex-col p-4">
          ...
          <div class="h-screen flex flex-col p-4">
            <div class="h-12 flex items-center justify-between">
              <div class="relative">
                ...
                <div @click="toggleSubmenu" ref="targetRef">
                  <div class="flex justify-between">
                    <div class="flex items-center pr-2">
                      ...
                      <div class="flex flex-col items-center justify-center">
                        <label class="relative inline-flex cursor-pointer items-center">
                          <input id="switch-3" type="checkbox" class="peer sr-only" v-model="isOn"
                            @change="handleToggle" />
                          <label for="switch-3" class="hidden"></label>
                          <div
                            class="peer h-4 w-11 rounded border bg-slate-200 after:absolute after:-top-1 after:left-0 after:h-6 after:w-6 after:rounded-md after:border after:border-gray-300 after:bg-white after:transition-all after:content-[''] peer-checked:bg-customSildboxred peer-checked:after:translate-x-full peer-focus:ring-green-300">
                          </div>
                        </label>
                      </div>
                    </div>
                  ...
                  <div v-if="isSubmenuVisible" class="absolute mt-2 bg-white border rounded shadow-lg w-48 right-5">
                    <ul class="text-gray-700">
                      <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer" @click="openModalSystemPrompt">
                        <h6>System Prompt</h6>
                      </li>
                      <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer" @click="openModalDelete">
                        <h6>Delete</h6>
                ...
                <!-- <button class="text-black font-bold px-4 py-2 rounded hover:bg-gray-200 transition"
                  @click="toggleSubmenu" ref="targetRef">
                  PROMPT
            ...
            <div class="flex flex-grow parent overflow-y-auto w-full">
              <div class="overflow-y-auto my-4 rounded-md w-full" ref="messagesContainer">
                <div
                  ...
                  <div v-for="(message, index) in chatRoomsList[selectIndexing].messages" :key="index"
                    :class="message.role">
                    <div v-if="message.type == 'text'" class="py-1">
                      <p v-for="(messager, idx) in message.content.split('\n')" :key="idx" :class="[
                        'text-black',
                        message.role === 'user'
                          ? 'bg-text_bg_color_message_response rounded-full px-4 py-6 my-1 inline-block font-semibold'
                          : ''
                      ]">
                        {{ messager }}
                      </p>
                    </div>
                    <img v-else :src="message.content" alt="image" class="w-40 h-40 object-cover rounded-md" />
                  </div>
                </div>
                ...
                <div v-else class="flex flex-col items-center justify-center h-full text-gray-400 py-10">
                  ...
                  <p class="text-2xl font-bold text-black mb-2">What can I help with?</p>
                  <div class="w-full max-w-xl flex mt-6">
                    <input type="text" class="flex-1 p-4 rounded-full bg-customeChat border-2 mr-2 text-black"
                      placeholder="Type your question..." v-model="inputMessage" @keydown.enter="handleSubmitMessage" />
                    <div
                      class="flex items-center border-2 rounded-full border-white p-3 outline-slate-500 bg-red_jetts cursor-pointer"
            ...
            <div
              v-if="chatRoomsList.length > 0 && chatRoomsList[selectIndexing]?.messages && chatRoomsList[selectIndexing].messages.length > 0"
              class="h-12 w-full flex">
              <div class="flex flex-grow">
                <input type="text" class="w-full p-4 rounded-full bg-customeChat border-2 mr-2"
                  placeholder="Input Here......" v-model="inputMessage" @keydown.enter="handleSubmitMessage" />
                <div
                  class="flex items-center border-2 rounded-full border-white p-3 outline-slate-500 bg-red_jetts cursor-pointer"
                  @click="handleSubmitMessage">
                  <Icon icon="material-symbols:send" width="24" height="24" class="text-white" />
              ...
              <div class="flex items-center border-2 rounded-full border-white p-3 outline-slate-500"
                @click="handleSubmitMessage">
                <Icon icon="material-symbols:send" width="24" height="24" class="text-white" />
              </div>
            </div>
        ...
        <!-- <div class="bg-slate-500 h-12 w-full flex">
          <div class="flex flex-grow">
            <input type="text" class="w-full p-4 rounded-full border-2 mr-2" placeholder="Input Here......"
              v-model="inputMessage" @keydown.enter="handleSubmitMessage" />
          </div>
          <div class="flex items-center border-2 rounded-full border-black p-3 outline-slate-500"
            @click="handleSubmitMessage">
            <Icon icon="material-symbols:send" width="24" height="24" class="text-white" />
          </div>
        </div> -->
  ...
  <Modal :isOpen="isModalSystemPromptOpen" title="System Prompt Edit" @close="closeModalSystemPrompt"
    ...
    <template #body>
      ...
      <input class="border-2  rounded-md w-full p-4 mb-2 " v-model="temperature" @input="validateInput"
        type="number" /> -->
      <div class="w-full max-w-[1200px]  p-7 rounded-3xl shadow-md">
        <div class="mb-8">
          <div class="flex items-center gap-8 mb-5">
            ...
            <input v-model.number="temperature" @input="validateInput" type="number" min="0" max="1" step="0.1"
        ...
        <div class="mb-8">
          ...
          <textarea class="border-2  rounded-md w-full p-4 h-[300px]" v-model="systemPrompt"></textarea>
        </div>
      </div>
    </template>
  </Modal>
...
onMounted(async () => {
  ...
  if (!settingStore.Settings.line_activate) {
    alert('Please activate LINE integration first.')
    router.push('/setting')
    return
  }
