<template>
  <div class="h-full flex flex-col p-4">
    <div class=" h-12 w-full flex items-center">
      <h3 class="text-back text-ls font-bold text-2xl">User Feedback</h3>
    </div>
        ...
        <table class="w-full border border-gray-200 rounded-lg overflow-hidden">
          <thead>
            <tr class="bg-gray-100">
              ...
              <th class="text-left p-3 border border-gray-300 font-bold">User</th>
          ...
          <tbody>
            <tr
              ...
              v-for="(doc, index) in Object.values(issues)"
              :key="index"
            >
...
const documentStore = useIssueStore()

const issues = computed(() => documentStore.issues)

onMounted(() => {
  documentStore.fetchIssues()
})

function handleEdit(id: string) {
  router.push({ name: 'editing-issue', params: { id: id } }).catch((err) => console.error(err))
}

function handleCreate() {
  router.push({ name: 'create-issue' }).catch((err) => console.error(err))
}
...
async function handleDelete(id: string) {
  ...
  try {
    await documentStore.deleteIssue(id)
  } catch (error) {
