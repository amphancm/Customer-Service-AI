...
def system_history_post(data):
    ...
    if isinstance(otg, str):
        ...
        assistant_content = otg.content
    else:
        assistant_content = str(otg)
    ...
    if len(result['messages']) < 3:
        try:
            ...
            if isinstance(res, str):
                ...
                title_name = res.content.replace('"', '')
            else:
            ...
            return {
                "title_name": '',
                "message": 'เกิดข้อผิดพลาดในการตั้งชื่อบทสนทนา กรุณาลองใหม่อีกครั้ง'
            }, 500
...
def line_callback(data):    # use this  route chat/line_callback
    print('enter line call back')
    ...
    line_bot_api = LineBotApi(setting['line_key']) # channel access token from line developer console
    handler = WebhookHandler(setting['line_secret']) # channel secret from line messaging API
